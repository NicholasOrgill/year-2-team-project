...
public class NetworkPlayScreen extends Screen {
	...
	/**
	 * Activates the opponent's power
	 * 
	 * @param key
	...
	/**
	 * Calculates the score when a note is hit
	 * 
	 * @param difference
	 *            The difference between the centre of the note and the centre
	 *            of the note hit area
	 * @param oppo
	 *            Flag as to whether the opponent or player hit the note
	 */
	public void scoreHelper(int difference, boolean oppo) {
		if (oppo) {
			if (getGameObject().getP2Text() != null) {
				if (getGameObject().getP2Text().equals("BAD")) {
					SystemTextCenterShake floatText = new SystemTextCenterShake((int) (getScreenWidth() * 0.75), 280,
							"BAD");
					floatText.shine();
					floatTexts.add(floatText);
				} else {
					SystemTextCenterFloat floatText = new SystemTextCenterFloat((int) (getScreenWidth() * 0.75), 280,
							getGameObject().getP2Text());
					floatText.shine();
					floatTexts.add(floatText);
				}
			}
			...
			if (difference <= getGameObject().PERFECT) {
				...
				sendCombo(combo);
				scoreQuality[0]++;
				SystemTextCenterFloat floatText = new SystemTextCenterFloat((int) (getScreenWidth() * 0.25), 280,
						"PERFECT");
				sendText("PERFECT");
				floatText.shine();
				floatTexts.add(floatText);
	...
	/**
	 * Handles when a bad note is hit
	 */
	public void bad() {
		if (power > 0)
			power--;
	...
	public void update() {
		...
		for (int i = n; i < notes.length; i++) {
			...
			if (noteSpriteLeft[i].isRemoved() == false) {
				if (noteSpriteLeft[i].getY() >= lineY
						&& noteSpriteLeft[i].getY() <= lineY + (3 * noteSpriteLeft[i].getHeight())) {
					boolean notesHit = true;
					for (int p = 0; p < notes[p].getButtons().length; p++) {
						if (notes[i].getButtons()[p]) {
							if (keys[p]) {
								hits.add(new NoteHitSprite(
										(playSpriteLeft.getX() - playSpriteLeft.getWidth() / 2)
												+ (playSpriteLeft.getBlockSizeAndGap() / 2)
												+ (p * playSpriteLeft.getBlockSizeAndGap()),
										playSpriteLeft.getY() + (playSpriteLeft.getBlockSize() / 2),
										playSpriteLeft.getBlockSize(), playSpriteLeft.getBlockSize()));
								keys[p] = false;
							} else {
								notesHit = false;
							}
		...
		for (int i = oppoN; i < notes.length; i++) {
			...
			if (noteSpriteRight[i].isRemoved() == false) {
				if (noteSpriteRight[i].getY() >= lineY
						&& noteSpriteRight[i].getY() <= lineY + (3 * noteSpriteRight[i].getHeight())) {
					boolean notesHit = true;
					for (int p = 0; p < notes[p].getButtons().length; p++) {
						if (notes[i].getButtons()[p]) {
							if (oppoKeys[p]) {
								oppoHits.add(new NoteHitSprite(
										(playSpriteRight.getX() - playSpriteRight.getWidth() / 2)
												+ (playSpriteRight.getBlockSizeAndGap() / 2)
												+ (p * playSpriteRight.getBlockSizeAndGap()),
										playSpriteRight.getY() + (playSpriteRight.getBlockSize() / 2),
										playSpriteRight.getBlockSize(), playSpriteRight.getBlockSize()));
								oppoKeys[p] = false;
							} else {
								notesHit = false;
							}
	...
	/**
	 * Sends a key press to the server
	 * 
	 * @param _key
	 *            The integer value of the key pressed
	 */
	...
	/**
	 * Sends a key release to the server
	 * 
	 * @param _key
	...
	/**
	 * Sends a power to the server
	 * 
	 * @param _power
	 *            The integer value of the power
	 */
	...
	/**
	 * Sends a combo to the server
	 * 
	 * @param _combo
	 *            The integer value of the combo
	 */
